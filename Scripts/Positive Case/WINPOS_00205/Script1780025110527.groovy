...
promo = 'Lvl 1 Promo Price - Lvl 2 Disc. Rp (Jam 10.00 - 13.00)'
...
'Load Excel'
TestData td = TestDataFactory.findTestData('Promotion Folder/Promo Multilevel - Level 1 and Level 2 Disc Rp Pkl 10 - 13')
...
if (expectedDiskon1 == actualDiskon1) {
    KeywordUtil.markPassed('✅ DISKON TRANSAKSI 1 SESUAI')
} else {
    KeywordUtil.markFailed('❌ DISKON TRANSAKSI 1 TIDAK SESUAI')
}
...
long expectedDiskon2 = td.getValue('diskon', 2).toLong()

String actualDiskonStr2 = Windows.getText(findWindowsObject('Transaksi Kasir/lbl_diskon_1'))

long actualDiskon2 = Math.abs(cleanNumber(actualDiskonStr2))

if (expectedDiskon2 == actualDiskon2) {
    KeywordUtil.markPassed('✅ DISKON TRANSAKSI 2 SESUAI')
} else {
    KeywordUtil.markFailed('❌ DISKON TRANSAKSI 2 TIDAK SESUAI')
}
...
for (int x = 3; x <= 4; x++) {
    ...
    if (expectedDiskon == actualDiskon) {
        KeywordUtil.markPassed('✅ DISKON TRANSAKSI 3 SESUAI')
    } else {
        KeywordUtil.markFailed('❌ DISKON TRANSAKSI 3 TIDAK SESUAI')
    }
}
...
String totalPembayaranStr3 = Windows.getText(findWindowsObject('Transaksi Kasir/txt_totalYangHarusDibayar'))

long totalPembayaranAct3 = cleanNumber(totalPembayaranStr3)

long expected3 = td.getValue('expected', 3).toLong()

long expected4 = td.getValue('expected', 4).toLong()

long grandTotalExpected3 = expected3 + expected4

if (grandTotalExpected3 == totalPembayaranAct3) {
    KeywordUtil.markPassed('✅ TOTAL PAYMENT 3 SESUAI')
} else {
    KeywordUtil.markFailed('❌ TOTAL PAYMENT 3 TIDAK SESUAI')
}

int nilai3 = Integer.parseInt(totalPembayaranStr3.replace(',', ''))

int totalAmount3 = Math.ceil(nilai3 / 100.0) * 100

String totalBelanja3 = totalAmount3.toString()

Windows.setText(findWindowsObject('Transaksi Kasir/txt_tunai'), totalBelanja3)
...
String pembayaranBerhasil3 = Windows.getText(findWindowsObject('Transaksi Kasir/txt_pembayaranBerhasil'))

if (pembayaranBerhasil3.length() > 0) {
    KeywordUtil.markPassed('✅ PAYMENT 2 BERHASIL')

    'Validasi Payment ke 3'
