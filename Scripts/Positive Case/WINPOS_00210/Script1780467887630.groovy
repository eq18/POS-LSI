...
promo = 'Promo Credit Card Min Belanja 500.000'
...
'Login'
Windows.callTestCase(findTestCase('Additional Cases/Login - E-Receipt'), [:], FailureHandling.STOP_ON_FAILURE)
...
'Load Excel'
TestData td = TestDataFactory.findTestData('Promotion Folder/Promo Credit Card Min 500')
...
Windows.click(findWindowsObject('Credit Card/btn_creditCard'), FailureHandling.STOP_ON_FAILURE)

Windows.delay(2)

Windows.takeScreenshot()

noKa = '1111222233334444'

'Input Nomor Kartu'
Windows.setText(findWindowsObject('Credit Card/txt_nomorKartu'), noKa)

Windows.sendKeys(findWindowsObject('Credit Card/txt_nomorKartu'), Keys.chord(Keys.ENTER))

'Klik tombol YA'
Windows.click(findWindowsObject('Credit Card/btn_yaPilihEDC'))

'Pilih Nama Bank'
Windows.click(findWindowsObject('Credit Card/cb_namaBankTipe'))

Windows.click(findWindowsObject('Credit Card/pilihTipeKartu'))

'Pilih EDC'
Windows.click(findWindowsObject('Credit Card/cb_EDC'))

Windows.click(findWindowsObject('Credit Card/pilihEDC'))
...
String expectedResult_fhbccd = td1.getValue('FHBCCD', 1)
...
String expectedStr_fhbccd = expectedResult_fhbccd.toString().trim().replace(',', '')
...
Windows.comment('Expected raw (FHBKAS)       : ' + expectedResult_fhbccd)

Windows.comment('Actual raw (total)          : ' + total)

Windows.comment('Expected clean (Payment Value): ' + expectedStr)

Windows.comment('Expected clean (FHBKAS)       : ' + expectedStr_fhbccd)
...
BigDecimal expected_fhbccd = new BigDecimal(expectedStr_fhbccd)
...
// ===== Validation 2: FHBCCD vs Total (optional) =====
if (expected_fhbccd.compareTo(actual) == 0) {
	Windows.comment("✅ FHBCCD sesuai | expected=$expected_fhbccd actual=$actual")
} else {
	Windows.comment("❌ FHBCCD tidak sesuai | expected=$expected_fhbccd actual=$actual")
}
...
'Validasi'
assert pdfText.contains('Hadiah Voucher Super Indo')
