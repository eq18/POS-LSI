...
'Load Excel'
TestData td = TestDataFactory.findTestData('Promotion Folder/Package Beli 3 Gratis 1')
...
'Validasi Diskon'
for (int row = 1; row <= 1; row++) {
    long expectedDiskon = td.getValue('diskon', row).toLong()
...
def td3 = findTestData('Query DB/Generate QR BNI')

String qrString = td3.getValue('qrstring', 1)

GlobalVariable.qrisStringBNI = qrString

GlobalVariable.total = nilai

println(GlobalVariable.qrisStringBNI)

println(GlobalVariable.total)

'Melakukan pembayaran di Simulator'
Windows.callTestCase(findTestCase('Additional Cases/BNI - Payment - QRIS'), [:], FailureHandling.STOP_ON_FAILURE)
