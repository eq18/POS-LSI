...
Windows.callTestCase(
		findTestCase('Additional Cases/Login'),
		[:],
...
promo = 'Promo Beli 5KG Lebih Hemat'
...
// ======================================================
Windows.click(
		findWindowsObject('Transaksi Kasir/btn_transaksiKasir')
)
...
TestData td =
		TestDataFactory.findTestData(
				'Promotion Folder/Promo Beli 5 KG Lebih Hemat'
		)
...
for (int row = 1; row <= 1; row++) {
	...
	// ==============================================
	Windows.click(
			findWindowsObject('Transaksi Kasir/txt_PLU')
	)
...
// ======================================================
Windows.click(
		findWindowsObject('Transaksi Kasir/btn_cekDiskon')
)
...
// ======================================================
long expectedDiskon1 =
		td.getValue('diskon', 1).toLong()

String actualDiskonStr1 =
		Windows.getText(
				findWindowsObject('Transaksi Kasir/lbl_diskon_1')
		)

long actualDiskon1 =
		Math.abs(
				cleanNumber(actualDiskonStr1)
		)

KeywordUtil.logInfo(
		'EXPECTED DISKON 1 = ' + expectedDiskon1
)

KeywordUtil.logInfo(
		'ACTUAL DISKON 1 = ' + actualDiskon1
)

if (actualDiskon1 == expectedDiskon1) {

	KeywordUtil.markPassed(
			'✅ DISKON PAYMENT 1 SESUAI'
	)

} else {

	KeywordUtil.markFailed(
			'❌ DISKON PAYMENT 1 TIDAK SESUAI'
	)
}
...
// ======================================================
Windows.click(
		findWindowsObject('Transaksi Kasir/btn_tutupCekDiskon')
)
...
Windows.click(
		findWindowsObject('Transaksi Kasir/btn_enter')
)
...
long totalPembayaranAct1 =
		cleanNumber(totalPembayaranStr1)

// ======================================================
// VALIDASI TOTAL PAYMENT 1
// ======================================================
long grandTotalExpected1 =
		td.getValue('expected', 1).toLong()

KeywordUtil.logInfo(
		'TOTAL EXPECTED 1 = ' + grandTotalExpected1
)

KeywordUtil.logInfo(
		'TOTAL ACTUAL 1 = ' + totalPembayaranAct1
)

if (grandTotalExpected1 == totalPembayaranAct1) {

	KeywordUtil.markPassed(
			'✅ TOTAL PAYMENT 1 SESUAI'
	)

} else {

	KeywordUtil.markFailed(
			'❌ TOTAL PAYMENT 1 TIDAK SESUAI'
	)
}
...
int totalAmount1 =
		Math.ceil(nilai1 / 100.0) * 100

String totalBelanja1 =
		totalAmount1.toString()

Windows.setText(
		findWindowsObject('Transaksi Kasir/txt_tunai'),
		totalBelanja1
)
...
for (int x = 2; x <= 2; x++) {
	...
	if (x == 2) {
		...
		actualDiskonStr =
				Windows.getText(
						findWindowsObject('Transaksi Kasir/lbl_diskon_2')
				)
	}

	long actualDiskon =
			Math.abs(
					cleanNumber(actualDiskonStr)
			)
...
long totalPembayaranAct2 =
		cleanNumber(totalPembayaranStr2)

// ======================================================
// VALIDASI TOTAL PAYMENT 2
// ======================================================
long expected2 =
		td.getValue('expected', 2).toLong()

long expected3 =
		td.getValue('expected', 3).toLong()

long grandTotalExpected2 =
		expected2 + expected3

KeywordUtil.logInfo(
		'TOTAL EXPECTED 2 = ' + grandTotalExpected2
)

KeywordUtil.logInfo(
		'TOTAL ACTUAL 2 = ' + totalPembayaranAct2
)

if (grandTotalExpected2 == totalPembayaranAct2) {

	KeywordUtil.markPassed(
			'✅ TOTAL PAYMENT 2 SESUAI'
	)

} else {

	KeywordUtil.markFailed(
			'❌ TOTAL PAYMENT 2 TIDAK SESUAI'
	)
}
...
if (pembayaranBerhasil2.length() > 0) {
	...
	Windows.click(
			findWindowsObject('Transaksi Kasir/Menu Utama')
	)
}
...
// ======================================================
Windows.callTestCase(
		findTestCase('Additional Cases/Logout'),
		[:],
		FailureHandling.STOP_ON_FAILURE)